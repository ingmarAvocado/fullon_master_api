...
class TestExampleOHLCVRoutes:
    ...
    async def test_ohlcv_client_methods_work(self, server_process):
        ...
        latest = await client.get_latest_ohlcv(exchange="kraken", symbol="BTC/USDC", timeframe="1m")
        ...
        trades = await client.get_trades(exchange="kraken", symbol="BTC/USDC", limit=5)

        # Should return list or None (trades endpoint may not exist)
        assert trades is None or isinstance(
            trades, list
...
class TestExamplePatternCompliance:
    ...
    def test_example_uses_http_api_not_direct_imports(self):
        ...
        assert (
            "httpx.AsyncClient" in content or "httpx" in content
        ...
        assert "from fullon_ohlcv_api import" not in content, (
            "Example should NOT import fullon_ohlcv_api directly. " "Use HTTP API via httpx instead"
        )

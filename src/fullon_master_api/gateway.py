...
class MasterGateway:
    ...
    async def lifespan(self, app: FastAPI):
        ...
        # Start configured services automatically
        if settings.service_auto_start_enabled:
            for service_name in settings.services_to_auto_start:
                try:
                    service_enum = ServiceName(service_name)
                    await self.service_manager.start_service(service_enum)
                    self.logger.info(f"Auto-started service: {service_name}")
                except Exception as e:
                    self.logger.error(f"Failed to auto-start service {service_name}", error=str(e))

        # Start HealthMonitor (it manages its own background monitoring task)
